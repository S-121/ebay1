      ...
      <template slot-scope="{data}">
        <tbody>
          <vs-tr :data="tr" :key="indextr" v-for="(tr, indextr) in data">
            ...
            <vs-td>
              <a :href="`https:\/\/www.ebay.com\/usr\/${tr.seller}`" target="_blank">
                <p class="product-category">{{ tr.seller}}</p>
              </a>
            </vs-td>
            <!-- <vs-td>
							<p>
            ...
            <vs-td>
              <vs-chip class="product-order-status">{{ tr.feedback}}</vs-chip>
            </vs-td>
            <!-- <vs-td>
							<vs-progress
            ...
            <vs-td>
              <p class="product-price">{{ tr.state }},{{tr.city}}</p>
            </vs-td>
...
export default {
  ...
  computed: {
    ...
    products() {
      var products = this.$store.state.products.analytic_items;
      var items = [];
      for (var i = 0; i < products.length; i++) {
        items[items.length] = {
          title: products[i].title,
          seller: products[i].seller.username,
          sales: products[i].item_details.quantity,
          feedback: products[i].seller.feedbackScore,
          price: products[i].price.value,
          state: products[i].item_details.location.stateOrProvince,
          city: products[i].item_details.location.city,
          image: products[i].image.imageUrl,
          url: products[i].itemWebUrl
        };
      }
      return items;
    },
